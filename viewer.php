...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #lyrics {
      font-size: calc(clamp(28px, 6vw, 96px) * var(--scale));
...
<body>
  ...
  <script>
    (function () {
      ...
      // ── Fullscreen ──────────────────────────────────────────────────────────
      // Hidden entirely where the API does not exist, rather than offering a control
      ...
      if (!document.documentElement.requestFullscreen) {
        ...
        fsItem.addEventListener('click', function () {
          if (document.fullscreenElement) {
            document.exitFullscreen().catch(function () { /* denied by the browser */ });
          } else {
      ...
      function armExpiry(ageMs) {
        ...
        expiryTimer = setTimeout(showNoText, remaining);
      }
      ...
      function connect() {
        ...
        ws.onmessage = function (event) {
          ...
          if (msg.action === 'musician_sync' && msg.data) {
            ...
            if (typeof msg.data.contentType !== 'string') return;
